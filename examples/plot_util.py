...
try:
    ...
    try:
        from examples.benchmark_proj import ProjectionResults  # noqa: F401
        from examples.benchmark_cvqp import BenchmarkResults  # noqa: F401
    except ImportError:
...
def plot_proj_benchmarks(results, save_figures=False):
    ...
    for tau in tau_values:
        ...
        for solver in solvers:
            solver_results = sorted([r for r in tau_results if r.solver == solver], key=lambda x: x.m)
...
def plot_cvqp_benchmarks(results, save_figures=False):
    ...
    for n_vars in n_vars_values:
        ...
        # Plot solution times
        for solver in solvers:
            solver_results = sorted([r for r in nvar_results if r.solver == solver], key=lambda x: x.n_scenarios)
